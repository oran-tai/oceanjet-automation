...
**Accommodation + Sex + station combo select (`_select_combo_verified`)**: pywinauto `select()` first (2 attempts, 1s apart), verified by reading the combo's Edit child back. If it doesn't land, first try typing the code into the focused combo (`_select_combo_by_typing`, drop-down-list combos only — Sex; incremental prefix search needs no UIA enumeration; strict read-back, never sends Enter; added Sept 25, 2026 after the Sex list was visibly open yet pywinauto saw no list items). Then click the combo's child `Button 'Open'` (fallback: Alt+Down on its Edit child) to physically drop the list (UIA Expand can silently no-op on Delphi combos — observed Sept 23, 2026: 3 failed attempts, dropdown never opened), click the ListItem, verify again. The booking-level absent-code error (`ACCOMMODATION_UNAVAILABLE` / `PASSENGER_VALIDATION_ERROR`) is raised only when an opened list was enumerated without the code; a list that never opens raises `RPA_INTERNAL_ERROR` (no 24h cooldown on a bookable booking). Sex combo passes `propagate_comerror=True` so COMError still reaches the sold-out check. Saves `debug/<label>_select_failed_*.png` on failure.

**Station dropdown retry**: `_select_station_with_recovery(trip_details, index, code, role)` selects via `_select_combo_verified` (same select() + physical-open + read-back path as accommodation/sex). If the opened list was enumerated without the station → `STATION_NOT_FOUND` immediately (no Gemini call). Any other failure (list never opened / value didn't verify) → classify the blocker via Gemini and dispatch: success_popup → raise `ORPHAN_TICKET_DETECTED`, print_preview → close, error_popup → dismiss + retry, none → retry anyway. The retry's own error propagates (`STATION_NOT_FOUND` if absent, `RPA_INTERNAL_ERROR` if the UI still can't be driven).
