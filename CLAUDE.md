...
**Accommodation combo select (`_select_accommodation`)**: pywinauto `select()` first (2 attempts, 1s apart), verified by reading the combo's Edit child back. If it doesn't land, click the combo's child `Button 'Open'` to physically drop the list (UIA Expand can silently no-op on Delphi combos — observed Sept 23, 2026: 3 failed attempts, dropdown never opened), click the ListItem, verify again. `ACCOMMODATION_UNAVAILABLE` is raised only when an opened list was enumerated without the code; a list that never opens raises `RPA_INTERNAL_ERROR` (no 24h cooldown on a bookable booking). Saves `debug/accommodation_select_failed_*.png` on failure.
