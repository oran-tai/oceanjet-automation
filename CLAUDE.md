...
**Date-fill blocker check (`_type_date_field`)**: 3 fill + read-back attempts; a mismatch usually means a modal popup is eating the keystrokes (observed: 'Session ID is missing' after Refresh when the PRIME session breaks). On failure: click Refresh (a broken session re-surfaces its popup), classify via `_classify_form_blocker()`, then dispatch — `error_popup`/`print_preview` → `UNEXPECTED_POPUP` (system-level: stops the loop, alerts both webhooks, engineer must re-login), `success_popup` → `ORPHAN_TICKET_DETECTED`, `none` → `PRIME_VALIDATION_ERROR` (booking-level). Never proceeds with an unverified date — there is no continue-on-unreadable fallback; the voyage-date guard in `select_voyage` stays as defense-in-depth for voyage-only mode (pax 2+).
...
**Accommodation + Sex + station combo select (`_select_combo_verified`)**: pywinauto `select()` first (2 attempts, 1s apart), verified by reading the combo's Edit child back. If it doesn't land, click the combo's child `Button 'Open'` (fallback: Alt+Down on its Edit child) to physically drop the list (UIA Expand can silently no-op on Delphi combos — observed Sept 23, 2026: 3 failed attempts, dropdown never opened), click the ListItem, verify again. The booking-level absent-code error (`ACCOMMODATION_UNAVAILABLE` / `PASSENGER_VALIDATION_ERROR`) is raised only when an opened list was enumerated without the code; a list that never opens raises `RPA_INTERNAL_ERROR` (no 24h cooldown on a bookable booking). Sex combo passes `propagate_comerror=True` so COMError still reaches the sold-out check. Sex only, as a last resort when the helper raises: `_select_sex_by_typing()` (set_focus, Escape if the list is dropped, type M/F, strict read-back, never Enter) — added Sept 25, 2026 after the list was visibly open yet pywinauto enumerated no items; the shared helper is untouched. Saves `debug/<label>_select_failed_*.png` on failure.

**Station dropdown retry**: `_select_station_with_recovery(trip_details, index, code, role)` selects via `_select_combo_verified` (same select() + physical-open + read-back path as accommodation/sex). If the opened list was enumerated without the station → `STATION_NOT_FOUND` immediately (no Gemini call). Any other failure (list never opened / value didn't verify) → classify the blocker via Gemini and dispatch: success_popup → raise `ORPHAN_TICKET_DETECTED`, print_preview → close, error_popup → dismiss + retry, none → retry anyway. If the retry still can't drive the list, stations only, as a last resort: `_select_station_by_typing()` (Escape if the list is dropped, click the combo's Edit child, Home/Shift+End, type the code, strict read-back, never Enter or Tab) — added Sept 26, 2026 after the origin list was visibly open yet pywinauto enumerated no items; station combos are editable (Edit + Open) and the voyage search is a separate button click, so no combo event is relied upon. The retry's own error propagates when typing doesn't verify (`STATION_NOT_FOUND` if absent, `RPA_INTERNAL_ERROR` if the UI still can't be driven). VM checks: `py tests\test_station_combo_keyboard.py` (TAG→CEB→TAG, no ticket) and `py tests\probe_combo_enumeration.py` (logs what pywinauto sees under the dropped list).
